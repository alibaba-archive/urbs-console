...
func NewDB() *SQL {
	...
	if cfg.User == "" || cfg.Password == "" || cfg.Host == "" {
		panic(fmt.Sprintf("Invalid SQL DB config %s:%s@(%s)/%s", cfg.User, cfg.Password, cfg.Host, cfg.Database))
	}

	parameters, err := url.ParseQuery(cfg.Parameters)
	if err != nil {
		panic(fmt.Sprintf("Invalid SQL DB parameters %s", cfg.Parameters))
	}
	// 强制使用
	...
	if err != nil {
		...
		panic(fmt.Sprintf("SQL DB connect failed %s, with config %s", err, url))
	}
